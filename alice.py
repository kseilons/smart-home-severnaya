...
from devices.devices import devicesService
...
log_handler = logging.FileHandler(config.LOG_FILE, 'a') if hasattr(config, 'LOG_FILE') else logging.StreamHandler()
...
def token():
    try:
        ...
        if request.form["code"] != last_code:
            logger.warning(f'invalid code. В запросе: {request.form["code"]}. последний {last_code}')
...
def action():
    try:
        ...
        for device in devices_request:
            ...
            new_device = devicesService.actionMethod(device, logger)
